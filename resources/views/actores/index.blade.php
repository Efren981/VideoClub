...
@section("content")
    <div class="row">
        <div class="col">
            <div class="row">
                <div class="col">
                    <h1 class="text-center mb-5">Actores</h1>
                </div>
            </div>
                        ...
                        <table class="table card" style="background: #e1e1e8">
                            ...
                            @foreach($datos as $director)
                                <tr scope="row">
                                    ...
                                    <td scope="col">{{$director->nombre_art}}</td>
                                    <td scope="col">{{$director->id_persona}}</td>
                                    <td scope="col"><a href="{{route("registro_director.edit",$director->id)}}" class="btn btn-success btn-sm" data-toggle="tooltip" title="Editar personas"><span class="icon-compose">Editar</span></a></td>
                                    <td scope="col"><form action="{{route("registro_director.destroy",$director->id)}}" method="post">
                                            @csrf
                                            @method('delete')
